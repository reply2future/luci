	...
	if node_id then
		...
		if local_socks_port then
			local inbound = {
				...
				sniffing = {enabled = sniffing and true or false, destOverride = {"http", "tls", "quic"}, routeOnly = route_only and true or nil, domainsExcluded = (sniffing and not route_only) and get_domain_excluded() or nil}
			}
		...
		local function gen_balancer(_node, loopbackTag)
			...
			if balancing_fallback_node and balancing_fallback_node ~= "" then
			...
			if #valid_nodes > 0 then
				...
				if balancer.strategy.type == "random" or balancer.strategy.type == "leastPing" or balancer.strategy.type == "leastLoad" then
